...
export default function AssignQR({ toast }) {
    ...
    const searchPerson = async (mobNum) => {
        const queryMobile = mobNum || mobile;
        if (queryMobile.length !== 10) return;
        setSearching(true);
        setError('');
        try {
            const res = await api.get(`/persons/search?mobile=${queryMobile}`);
            if (res.data.found) {
